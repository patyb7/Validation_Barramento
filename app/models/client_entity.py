...
from datetime import datetime, timezone # CORRIGIDO: Importar timezone de datetime
import logging # NOVO: Adicionar import para logging

logger = logging.getLogger(__name__) # NOVO: Inicializar logger
...
class ClientEntity(BaseModel):
    ...
    """
    id: str = Field(default_factory=lambda: str(uuid.uuid4()), description="ID único da entidade cliente (UUID).")
    ...
    golden_record_cpf_cnpj_id: Optional[int] = Field(None, description="ID do GR do CPF/CNPJ principal.")
    golden_record_address_id: Optional[int] = Field(None, description="ID do GR do Endereço.")
    golden_record_phone_id: Optional[int] = Field(None, description="ID do GR do Telefone.")
    golden_record_email_id: Optional[int] = Field(None, description="ID do GR do Email.")
    golden_record_cep_id: Optional[int] = Field(None, description="ID do GR do CEP.")
    ...
    def update_golden_record_id(self, validation_type: str, record_id: int):
        """
