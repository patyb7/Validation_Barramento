# app/models/qualificacao_pendente.py
...
class QualificacaoPendente(BaseModel):
    ...
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Timestamp de criação do registro.")
    updated_at: datetime = Field(default_factory=datetime.utcnow, description="Timestamp da última atualização do registro.")

    class Config:
        from_attributes = True # Permite que o modelo seja criado a partir de atributos de objeto
        populate_by_name = True # Permite mapeamento por nome de campo, útil para campos com alias
        json_encoders = {
...
class InvalidosQualificados(BaseModel):
    ...
    archived_at: datetime = Field(default_factory=datetime.utcnow, description="Timestamp de quando o registro foi arquivado como inválido.")
