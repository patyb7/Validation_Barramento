...
try:
    ...
    # Removido PhoneNumberType.SHORT_CODE, .STANDARD_RATE, .EMERGENCY, .NO_TYPE
    # que não existem na versão 8.13.x
    PHONE_TYPE_MAP.update({
        ...
        # phonenumbers.PhoneNumberType.SHORT_CODE: "Código Curto", # <-- REMOVIDO
        # phonenumbers.PhoneNumberType.STANDARD_RATE: "Tarifa Padrão", # <-- REMOVIDO
        # phonenumbers.PhoneNumberType.EMERGENCY: "Emergência", # <-- REMOVIDO
        # phonenumbers.PhoneNumberType.NO_TYPE: "Sem Tipo", # <-- REMOVIDO
    })
...
class PhoneValidator(BaseValidator): # Herda de BaseValidator
    """
    ...
    def __init__(self):
        super().__init__(origin_name="phone_validator") # Inicializa o BaseValidator
    ...
    def _get_phone_type_name_safe(parsed_number_obj) -> str:
        ...
        if PHONENUMBERS_AVAILABLE and parsed_number_obj:
            ...
            return PHONE_TYPE_MAP.get(num_type, f"Tipo Desconhecido ({num_type})") # Ajustado para .name caso exista, mas (num_type) é mais seguro para todas as versões
    ...
    async def validate(self, data: Any, country_hint: Optional[str] = None, **kwargs) -> Dict[str, Any]: # <--- country_hint AGORA É ARGUMENTO EXPLÍCITO
        """
        ...
        Args:
            data (Any): O número de telefone a ser validado. Esperado uma string.
            country_hint (Optional[str]): Uma dica de código de país (ISO 3166-1 alpha-2, ex: "BR", "US").
        ...
        # cleaned_data passa a ser dado_normalizado no retorno
        ...
        if PHONENUMBERS_AVAILABLE:
            try:
                ...
                if details["phonenumbers_valid"]:
                    ...
                    # Continue para as regras de fallback. O resultado será definido abaixo.
                    pass
                else:
        ...
        if len(digits_only) == 3 and digits_only.isdigit():
            if digits_only in SERVICE_EMERGENCY_NUMBERS_BR:
                ...
                details["country_code_detected"] = 55 # Assuming BR emergency numbers
                try:
        ...
        if normalized_number.startswith('+'):
            ...
            if match:
                ...
                if 7 <= len(national_number_part) <= 15: # Typical E.164 national number length
        ...
        # Use digits_only for Brazilian length and DDD rules
        ...
        if is_br_hint or looks_like_br:
            if 10 <= len(digits_only) <= 11:
                ...
                if not self._is_brazilian_ddd_valid(ddd):
                    ...
                    if len(digits_only) == 11 and numero_principal.startswith('9'):
                        ...
                        return self._format_result(
                            is_valid=True,
                            dado_normalizado=f"+55{digits_only}", # Ensure E.164 for BR
                    ...
                    elif len(digits_only) == 10 and numero_principal[0] in ('2', '3', '4', '5'): # Common fixed-line prefixes
            ...
            else: # Invalid length for Brazilian number
                details["country_code_detected"] = 55
                try:
                    details["national_number"] = int(digits_only)
                except ValueError:
                ...
                return self._format_result(
                    ...
                    details=details,
                    business_rule_applied={"code": VAL_PHN012, "type": "phone"} # Reused for invalid BR length
                )
        # General case: number did not fit any specific rule
        return self._format_result(
            is_valid=False,
            dado_normalizado=normalized_number,
