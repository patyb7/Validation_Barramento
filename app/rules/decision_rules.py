...
Regras de Ação com Tratamento de Erros Integrado: A inclusão de _rule_soft_delete_invalid_records e _rule_check_duplicates com blocos try-except e logging detalhado.

Vantagem: Garante que as operações (como soft delete ou verificação de duplicidade) sejam executadas de forma controlada. O tratamento de exceções evita falhas na aplicação das regras e o logging fornece um registo claro de todas as ações tomadas e seus resultados, mesmo em caso de erro.
